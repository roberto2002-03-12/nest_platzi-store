...
export class ProductService {
  ...
  findOne(id: number) {
    const product: Product | undefined = this.products.find(pro => pro.id === id)
    if (!product) throw new NotFoundException(`Product with id ${id} not found`);
    return product;
  }

  create(payload: CreateProductDto) {
    const id: number = this.products.length > 0 
  ...
  update(id: number, payload: UpdateProductDto) {
    ...
    if (indexOfProduct === -1) throw new NotFoundException(`Product with id ${id} not found`);
